...
create_permax_quants_labs <- function(df, fcaststartdate, fcastenddate, yeartrace, value, qtype)    {
  ...
  df  %>% filter(fcast_t_pst >= fcaststartdate, fcast_t_pst <= fcastenddate) %>%
    ...
    summarize(
      ...
      `median peak`= quantile(maxtraceval, probs=0.50, type = qtype)) %>% gather(key = lab, value = val)
...
create_permax_quants_i <- function(df, fcaststartdate, fcastenddate, yeartrace, value, qtype, fcast_i_pst)    {
  ...
  df  %>% filter(fcast_t_pst >= fcaststartdate, fcast_t_pst <= fcastenddate) %>%
    ...
    summarize(
      ...
      `90%`=quantile(maxtraceval, probs=0.9, type = qtype),
      `95%`=quantile(maxtraceval, probs=0.95, type = qtype),
      `min`=min(maxtraceval), `max`=max(maxtraceval), mean=mean(maxtraceval), 
      median=quantile(maxtraceval, probs=0.5, type = qtype), 
      n=n()) %>% mutate(qtype = qtype, fcast_i_pst_hms = fcast_i_pst, fcast_i_pst_date = date(fcast_i_pst_hms))
}


create_permax_quants_sumtable_f <- function(df, fcaststartdate, fcastenddate, yeartrace, value, qtype)    {
  ...
  df  %>% filter(fcast_t_pst >= fcaststartdate, fcast_t_pst <= fcastenddate) %>%
    ...
    summarize(
      ...
      `90%`=quantile(maxtraceval, probs=0.10, type = qtype),
      `75%`=quantile(maxtraceval, probs=0.25, type = qtype),
      `50% (median peak)`=quantile(maxtraceval, probs=0.50, type = qtype),
      `25%`=quantile(maxtraceval, probs=0.75, type = qtype),
      `10%`=quantile(maxtraceval, probs=0.9, type = qtype),
      `5%`=quantile(maxtraceval, probs=0.95, type = qtype),
      `(max peak, all traces)`= max(maxtraceval)) %>% 
    gather(key = `Entire Period Selected, Chance of Exceeding:`, value = `Flow (kcfs)`)%>%
    mutate(`Flow (kcfs)` = round(`Flow (kcfs)`, 2))
  
}


create_permax_quants_sumtable_s <- function(df, fcaststartdate, fcastenddate, yeartrace, value, qtype)    {
  ...
  df  %>% filter(fcast_t_pst >= fcaststartdate, fcast_t_pst <= fcastenddate) %>%
    ...
    summarize(
      ...
      `5%`=quantile(maxtraceval, probs=0.95, type = qtype),
      `(max peak, all traces)`= max(maxtraceval)) %>% 
    gather(key = `Entire Period Selected, Chance of Exceeding:`, value = `Stage (ft)`)%>%
    mutate(`Stage (ft)` = round(`Stage (ft)`, 2))

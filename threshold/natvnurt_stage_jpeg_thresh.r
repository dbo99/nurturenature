...
p1 <- ggplot() + 
  ...
  geom_hline(yintercept = monitorstage_feet, linetype = "dashed", color = "deeppink", show.legend = F, size = 2.000) + 
  ...
  geom_label(data = filter(notify_levs, unit == "feet"), aes(x = midpoint, y = value, label = paste0(type," (",value_t,")")), show.legend = F, alpha = 0.6) +
